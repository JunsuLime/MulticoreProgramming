...
/* multicore
 * tip for THD object and thread pool
 *
 * thread pool has physical thread
 * and THD is logical thread.
 * so, physical context switching time decrease ...
 * And, keeping logical thread(THD)'s context, we must do worker_context.save();
 * in worker_context.save function, THR_KEY_mysys is global key for logical thread.
 * pthread_getspecific -> return memloc of thread local var
 * pthread_setspecific(KEY, memloc) -> set return of getspecific to memloc
 *
 * Anyway... when we deal with THD object, we must use worker_context and save(),
 * restore() function.
 *
 * explanation of threadpool_process_request()
 * 
 * save worker_context and do one's job
 * context save is needed because of local thread var (ex- mutex of thread)
 * execute do_command function
 * do_command function execute single query
 * in my thinking ... this is from begin trx to end trx.
 * end of this job, restore context is done.
 *
