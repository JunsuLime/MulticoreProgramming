...
bool do_command(THD *thd)
{
  ...
  */
   ...
   * dispatch_command
   *
   * execute query or simple command
   * in dispatch_command
   * do process and send OK !!
   */
...
out:
  /* multicore
   * this commands don't need to be modified
   *
   * because, do_command function is called until end of packet,
   * lex is neeed to be restore to reuse
   * LEX lex: The state of the lex parsing.
   */
  thd->lex->restore_set_statement_var();
		      ...
		      char* packet, uint packet_length)
{
  ...
  */
  ...
  case COM_QUERY:
  {
           ...
           ! thd->is_error())
    {
      ...
      */
      printf("multiple query exist case\n");
 ...
 dispatch_end:
  ...
  /* multicore
   * About send OK section
   *
   * do_end_of_statement is initialized to true and we don't consider WSREP option.
   * so do_end_of statement is always true
   * 
   * when os_flush doing case, make do_end_of_statement=false;
   * let's find which case os_file_flush is done !!
   *
   * although ha_trans_commit case.. os_file_flush did not occured in this case
   * so we must check file_flush is done!!!
   * 
   * i think that flag will be embedded in THD object.
   * because THD object is carried to backend innobase_commit function.
   */
  if (do_end_of_statement && thd->send_flag == FALSE)
  {
