...
/* multicore
 *
 * callback fucntion implementation
 */
void put_connection_on_poll(connection_t *c);
...
void put_connection_on_poll(connection_t *c) {
    //printf("put_connection_on_poll is called (threadpool_unix)\n");
    set_wait_timeout(c);
    int err = start_io(c);
...
/* multicore
 * handle_event: handle event that connection holds
...
static void handle_event(connection_t *connection)
{
  ...
  //printf("handle_event is called\n");

  /* multicore
   *
   * THD's send_flag initialize
   */
  connection->thd->send_flag = FALSE;
  connection->thd->is_trx_end = FALSE;
  __sync_synchronize();
   ...
   * register fd in epoll again
   */
  if (connection->thd->send_flag == FALSE) {
    //printf("check point0\n");
    set_wait_timeout(connection);
    err= start_io(connection);
  }
  else {
    //printf("turn on is_trx_end flag\n");
    connection->thd->is_trx_end = TRUE;
    __sync_synchronize();
  }
end:
    if (err)
