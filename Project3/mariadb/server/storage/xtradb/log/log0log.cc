...
/*==========*/
{
    ...
    while(!log_sys->trx_table.empty()) { 
        printf("WHEN INIT !! POP IT!!!\n");
        log_sys->trx_table.pop();
    }
...
#ifdef PROJECT3
    // printf("log_write_up_to fucntion called\nlsn: %lu \n", lsn);
#endif
...
# endif
     ...
     * About log_t 
     *
     * flushed_to_disk_lsn: (storage)lastest flushed disk lsn
     * written_to_all_lsn:
     * written_to_some_lsn:
     * n_pending_writes: number of currently pending flush or write
     * current_flush_lsn: (storage)end lsn for the current running flush
     * write_lsn: (log file)write log start lsn
     * buf_next_to_write:
     * lsn:
     * 
     */
     ...
     * case 1
     *
     * already done by other things...
     * 1) log buffer -> log file write
     * 2) disk flush
     * 
     * so, don't need to other things.
     * Do not modify this section
     *
     * question? - flush_to_disk is needed?? why it exist?
     * let's consider flush_to_disk false case...
     * 
     */
    ...
    /* multicore
     * case that pending write thread exist
     * 
     * In this case, wait for other thread's file flush
     * this section ......
     * modify do_waits to register callback
     */
	...
	if (log_sys->n_pending_writes > 0) {
         ...
         * flush_to_disk flag ...
         *
         * this flag - i will flush storage
         * but current called flush lsn doing ... so i don't have to do anything.
         * just wait
         */
		if (flush_to_disk
         ...
         * flush_to_disk flag ...
         *
         * ! this flag - i will write log file only
         * but current called flush lsn doing ... so i don't have to do anything
         * just wait
         */
...
#ifdef PROJECT3
    /* multicore
     * after flush !! send ok to client
     * flush is only done by flush thread (this requirent must be satisfied)
     * in innobase_mysql_log_notify function, checkpoint is reserved
     * after get checkpoint of db, send OK to client
     */
    if (!flush_to_disk) {
        return true;
    }
    // printf("\n*********************************\nlog_write_up_to [flush] is done\n");
    // TODO: find grained lock implementation
    pthread_mutex_lock(&log_sys->pq_mutex);
    __sync_synchronize();
    while(!log_sys->trx_table.empty())  {
        // get trx from log_sys's priority queue
        wrap_trx w_trx = log_sys->trx_table.top();
        trx_t *t = w_trx.trx;
        //printf("thd's trx_end_flag:%d\n",thd_check_is_trx_end(t.mysql_thd));
        if (w_trx.lsn <= flush_lsn && thd_check_is_trx_end(t->mysql_thd) == TRUE) {

            log_sys->trx_table.pop();

            // send OK to client (trx->mysql_thd's)
            trx_send_ok_thd(t->mysql_thd);
            trx_put_connection_on_poll(t->mysql_thd);
            printf("", w_trx.lsn);
        }
        else {
            break;
    ...
    pthread_mutex_unlock(&log_sys->pq_mutex);
#endif
	return false;
	...
	ibool	flush)	/*!< in: flush the logs to disk */
{
    ...
    /* multicore
     * background threed must do flush
     */
//    printf("log_buffer_sync_in_background_function is called\n");
    flush = TRUE;
