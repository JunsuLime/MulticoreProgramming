...
struct trx_t{
	...
	* NOT_STARTED -> PREPARED -> (freed)
    ...
    /* multicore
     * trx_t object has THD object corresponding handle thread
     *
     * so get trx_t -> until fil_flush() function.
     * if fil_flush is done!!, mysql_thd->flush_flag = true;
     * frontend see the flag and wow !! flush is working !! go other work. finish
     * fil flush is replaced to register callback.
     */
...
struct wrap_trx {
    ...
    /* multicore
     * 
     * lsn is for key value of compare
     */
    lsn_t lsn;
    bool operator > (const wrap_trx &obj) const { return lsn > obj.lsn; }
};
