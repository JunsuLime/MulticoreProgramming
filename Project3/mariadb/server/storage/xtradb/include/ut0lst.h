...
/*******************************************************************//**
* multicore
* Adds the node as the first element in a two-way linked list as lockfree.
* @param list	the base node (not a pointer to it)
* @param elem	the element to add
* @param offset	offset of list node in elem.
* Use '__sync_bool_compare_and_swap' operation to implement lock free																	  
*/
template <typename List, typename Type>
void
ut_list_prepend_lockfree(
	List&		list,
	Type&		elem,
	size_t		offset)
{
	while(true){
		...
		Type* head = list.start;
		ut_list_node<Type>&	elem_node = ut_elem_get_node(elem, offset);

		elem_node.prev = 0;
		if(!__sync_bool_compare_and_swap(&list.start, head, &elem)) {
			continue;
		}
		
		if (head != 0) {
			ut_list_node<Type>&	base_node =
				ut_elem_get_node(*head, offset);
		...
		__sync_bool_compare_and_swap(&list.end, 0, &elem);
		//++list.count;
...
/* multicore
 * 
 * Define list add first lockfree
 *
 * end
 */
#define UT_LIST_ADD_FIRST_LOCKFREE(NAME,LIST,ELEM)\
	ut_list_prepend_lockfree(LIST,*ELEM, IB_OFFSETOF(ELEM,NAME))
