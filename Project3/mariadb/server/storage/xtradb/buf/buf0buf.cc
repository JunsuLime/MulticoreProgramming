	...
	byte*		frame)		/*!< in: pointer to buffer frame */
{
	...
	pfs_register_buffer_block() */
/* multicore
 * mutex and rw_lock_create part
 *
 * This part is related to mutex, rw_lock_create.
 *
 * end
 */
...
UNIV_INTERN ib_mutex_t mutex_test;
#ifdef PROJECT3
/* multicore
 * Make structure for pthread_create
 * each parameter buf_pool_t* pool, ulint size, int index
 * Since buf_pool_init_instance's paramter is 3.
 * So make structure for buf_pool_init_instance's parameter.
 * end
 */
 ...
 * Make Thread func 
 * @param : structure
 * Put above structure in paramter to send to buf_pool_init_instance's parameter.
 * end
 */
void* thread_pool_init_instance(void* arg){
	printf("tttttttttttttttttttttttttt\n");
	thread_stuff* stuff = (thread_stuff*)arg;
	buf_pool_t* pool = stuff->pool;
	if (buf_pool_init_instance(stuff->pool, stuff->size, stuff->index) != DB_SUCCESS) {

			/* Free all the instances created so far. */
			buf_pool_free(stuff->index);
			printf("buf_pool_init_instance error\n");
			return NULL;
		}
	delete stuff;
	return pool;
	...
	ulint	n_instances)	/*!< in: number of instances */
{
	...
	printf("buffer pool instances : %lu \n",n_instances);
	printf("total size : %lu\n",total_size);
	printf("size : %lu\n",size);
	/* multicore
	* multi thread create
	* each thread works for each instance(chunk)
	* make structure to assign argument in pthread_create's parameter.
	* end
	*/

#ifdef PROJECT3
	printf("with multithread\n");
	pthread_t* threads = new pthread_t[n_instances];
	printf("with multicore \n");
	for (i = 0; i < n_instances; i++) {
		buf_pool_t*	ptr	= &buf_pool_ptr[i];
		
		thread_stuff* stuff = new thread_stuff();
		stuff->pool = ptr;
		stuff->size = size;
		stuff->index = i;
		//mutex_enter(&mutex_test);
		pthread_create(&threads[i], NULL, thread_pool_init_instance, (void*)stuff);
		//mutex_exit(&mutex_test);
	}
	for(i = 0; i < n_instances; i++) {
		void* results;
