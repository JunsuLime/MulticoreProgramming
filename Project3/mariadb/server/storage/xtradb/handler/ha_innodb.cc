...
/* multicore
 * align chunks based on available cores
 * end
 */
#include <unistd.h>
...
/* multicore
 * 
 * thd's send_flag on function
 * thd_set_lock_wait_time() func is 
 * motive for this function implementation
 */ 
void
thd_send_flag_on(
    void* thd)
{
    thd_send_flag_on_low((THD*) thd);
}

/* multicore
 * 
 * trx_end flag is needed
 */
bool
thd_check_is_trx_end(
    void* thd)
{
    return thd_check_is_trx_end_low((THD*) thd);
}

/* multicore
 * 
 * put connection on epoll
 */
void
trx_put_connection_on_poll(
    void* thd)
{
    //printf("trx_put_connection_on_poll\n");
    trx_put_connection_on_poll_low((THD*) thd);
}

/* multicore
 * 
 * send ok to client
 * In log0log.cc file, to use thd's end protocol
 */
void 
trx_send_ok_thd(
    void* thd)
{
    //printf("trx_send_ok_thd is called\n");
    trx_send_ok_thd_low((THD*) thd);
}
...
innobase_change_buffering_inited_ok:
	...
	printf("buf instance : %lu\n",innobase_buffer_pool_instances);
	if (innobase_buffer_pool_instances == 0) {
		printf("inside innobase_buffer_pool_instance : %lu\n",innobase_buffer_pool_instances);
		/* multicore
		 * number of instances = number of core
		 * end
		 */
		innobase_buffer_pool_instances = sysconf(_SC_NPROCESSORS_ONLN);
		//innobase_buffer_pool_instances = 8;
		printf("innobase_buffer_pool_instances : %lu\n",innobase_buffer_pool_instances);
