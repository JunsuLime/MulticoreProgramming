	...
	ulint		cline)		/*!< in: file line where created */
{
	...
	mutex_create(rw_lock_mutex_key, rw_lock_get_mutex(lock),
		     SYNC_NO_ORDER_CHECK);
	printf("mutex in rw_lock_create_func created!\n");
...
# endif
	...
	ut_ad(UT_LIST_GET_LEN(rw_lock__list) == 0
		  || UT_LIST_GET_FIRST(rw_lock_list)->magic_n == RW_LOCK_MAGIC_N);

	/* multicore
	 *
	 * Use Lockfree add list
	 *
	 * end
	 */
	UT_LIST_ADD_FIRST_LOCKFREE(list, rw_lock_list, lock);
/*
