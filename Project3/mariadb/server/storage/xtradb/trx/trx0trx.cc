...
/* multicore
 * trx_flush_log_if_needed_low modification
 */
	...
	trx_t*	trx)	/*!< in: transaction */
{
    ...
    /* multicore
     * flush_log_at_trx_commit is set in my.cnf
     * it has option
     * 0:
     * 1:
     * 2:
     ...
     * and !!
     * modification is needed in this function
     */
	...
	case 1:
        ...
        skip = log_write_up_to(lsn, LOG_WAIT_ONE_GROUP,
				srv_unix_file_flush_method != SRV_UNIX_NOSYNC);
		break;
	case 2:
		/* Write the log but do not flush it to disk */
		skip = log_write_up_to(lsn, LOG_WAIT_ONE_GROUP, FALSE);
...
#ifdef PROJECT3
    /* after log_write_up_to();
     *
     * 1) send_ok_flag - flag on !! (trx->mysql_thd->flag on!)
     * 2) register trx in callback table (key: lsn, value: trx)
     * in case of select query, we don't need to commit that comman 
     */ 
     if (trx->mysql_thd != NULL && skip) {
        //printf("latency hiding ......\n")
        // 1) send_ok_flag on
        thd_send_flag_on(trx->mysql_thd);

        // 2) trx->lsn = lsn; and log_sys's callback table
        pthread_mutex_lock(&log_sys->pq_mutex);
        struct wrap_trx w_trx;
        w_trx.trx = trx;
        w_trx.lsn = lsn;
        log_sys->trx_table.push(w_trx);
        __sync_synchronize();
        pthread_mutex_unlock(&log_sys->pq_mutex);
        //printf("\n***********************\ntrx_flush_log_if_needed_low\nregister trx in table\nlsn: %ld\n****************************\n", lsn);
     }
#endif
}
...
/*
// useless code
// do not check macro with #ifdef during class time
#ifdef PROJECT3_PART1
    long t = 0;
	...
	trx_t*	trx)	/*!< in/out: transaction */
{
    ...
    /* multicore
     * 
     * this function (thd_flush_log_at_trx_commit) is motive for register
     * flag function in handler
     */
